...
async function recordInteraction(userId, interactionType, sentimentChange) {
  try {
    console.log(
      `Recording: user=${userId}, type=${interactionType}, sentiment=${sentimentChange}`,
    );

    const { data, error } = await supabase.rpc("record_interaction", {
      p_user_id: userId,
      p_interaction_type: interactionType,
      p_sentiment_change: sentimentChange,
    });
    ...
    console.log(`Interaction recorded for user ${userId}`);
  } catch (error) {
